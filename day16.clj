...
(let [Map (parse "input")
      ...
      [fin data] (loop [todo #{[start 0]}
                   ...
                   (let [[pos ndir :as doing] (apply min-key #(get-in data [% :score]) todo)
                         ...
                         newdata (for [k newpos]
                                   (let [score (+ curscore (if (= ndir (k 1)) 1 1000))]
                                     [k {:from #{doing} :score score}]))]
                     ;(println doing)
                     (if (and forw (= end (forw 0)))
                       [forw
                        (reduce #(let [[k v2] %2]
                                   (if-let [v1 (get %1 k)]
                                     (assoc! %1 k (mymerge v1 v2))
                                     (assoc! %1 k v2))) data newdata)]
                       (let [newdone (conj done doing)]
                         (recur (set/difference (reduce conj (disj todo doing) newpos) newdone)
                                newdone
                                (reduce #(let [[k v2] %2]
                                           (if-let [v1 (get %1 k)]
                                             (assoc! %1 k (mymerge v1 v2))
